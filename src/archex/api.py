...
def _ensure_index(
    ...
    # Path 2: Delta path — same repo, different commit (local only; URL repos are always fresh clones)
...
def _bootstrap_plugins(strict: bool = False) -> None:
    ...
    if _PLUGIN_BOOTSTRAP_STRICT is not None:
        if not strict or _PLUGIN_BOOTSTRAP_STRICT:
