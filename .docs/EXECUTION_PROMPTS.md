...
RECONCILIATION RULE: R6.1-DG-003 is material. Open `docs(plan): reconcile R6.1 OpenRouter output accounting` as a docs-only prerequisite containing the matching plan and prompt update; it must be reviewed, green, and externally merged before the replacement pre-registration, fixture, code, or data PR. After it merges, rerun this gate and require `DESIGN GO — PLAN REVISION: none`.

PLANNED STACK:
0. Conditional prerequisite `docs(plan): reconcile R6.1 OpenRouter output accounting` — scope: authoritative plan and execution prompt only; must externally merge before PR-1.
