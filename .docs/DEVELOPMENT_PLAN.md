...
| Field | Value |
| --- | --- |
| Objective | Measure whether archex's unchanged deterministic ordering changes OpenRouter-metered, Anthropic-upstream input-side prefix-cache cost per fixed resolved maintenance session, using only a fresh, cache-eligible fixture. OpenRouter may return enforced response tokens despite `max_tokens: 0`; the study records them but derives its metric exclusively from the provider-observed `cost_details.upstream_inference_prompt_cost` and never feeds response content into a later request. This is an independent, post-Gate-A study authorization; it cannot reopen Gate A, revive R6 or PR #593, or support a product or retrieval-quality claim. |
| In / Out of scope | In: one pre-registered three-turn, `token_budget: 8192` session for each of 12 independent repositories: the self-repo plus at least one task from every current external corpus family, selected from existing pinned tasks; shipped-default retrieval to freeze the same selected context and fixed resolution labels for all arms; deterministic, seed-recorded perturbed, and seed-recorded ANN-style ordering of those same chunks; an OpenRouter-only benchmark client that pins `anthropic/claude-opus-5` to the `anthropic` upstream with fallbacks disabled and uses explicit cache control; OpenRouter prewarm/replay and usage receipts; paired repository-cluster bootstrap. Out: a product retrieval or ordering change, live ANN retrieval, retrieval-quality or task-resolution comparisons, local token estimates as eligibility proof, response-token cost or content in the metric or session history, R6's fixture or closed artifact, use of the superseded OpenRouter pre-registration in PR #598, a reusable provider abstraction before a second provider cell exists, and any successor to cancelled R7–R16. |
| Depends on | `R1`; R6's no-go record is a non-execution constraint, not a reusable predecessor artifact. |
| Target release | `unversioned` |
| Deliverables | A new immutable pre-registration at `benchmarks/preregistrations/S7-determinism-economics-r6.1-openrouter-output.md` superseding the unrun first-party protocol in PR #596 and the unrun zero-output OpenRouter protocol in PR #598; it names OpenRouter model `anthropic/claude-opus-5`, `provider.only: ["anthropic"]`, `allow_fallbacks: false`, explicit `cache_control`, the OpenRouter model/pricing and cache-usage source URLs with retrieval timestamps, the observed response-output constraint, the pre-registered cache-write eligibility rule, a `+5%` minimum worthwhile gain (MWG), a `-5%` non-inferiority margin (NIM), and a `[-5%, +5%]` equivalence region (EQM). The fixture remains provider-neutral and contains frozen rendered bytes, SHA-256 values, orders, turns, source revisions, and labels. Committed receipts carry the rendered-prefix SHA-256, model, requested routing policy, resolved `provider_name`, generation ID, prompt and completion usage fields, provider-observed input cost, nonzero cache-write tokens for every prewarm, nonzero cache-read tokens for every replay, source revision, and fixture digest; a dedicated ordering-economics runner, JSON validator, artifact, and decision document use this one registered cell. |
| Acceptance | The replacement OpenRouter pre-registration merges before fixture construction or data generation; PR #596's unrun direct-Anthropic pre-registration and PR #598's unrun zero-output OpenRouter pre-registration are preserved and explicitly retired. Each arm uses `anthropic/claude-opus-5`, is served by `provider_name: "Anthropic"` with no fallback response, and places the cache breakpoint after identical frozen context bytes. Every rendered prefix has an exact-SHA prewarm/replay pair with a nonzero cache write on prewarm and a nonzero cache read on replay; prewarm cache-write usage establishes provider-accepted eligibility for that exact prefix. A missing, stale, mismatched, routed-to-other-provider, fallback, or zero receipt fails validation. The metric uses only the provider's prompt-cost field; recorded response tokens and their cost are excluded and response content never enters later request history. All arms use the same session IDs, source chunks, labels, model-price schedule, and turn content, differing only in context order. The artifact rejects an undeclared or non-fired arm and labels every decision cell `superior`, `equivalent`, or `inconclusive at this N`; it never pools provider/model cells or makes a product or retrieval-quality claim. |
| Verification | With `OPENROUTER_API_KEY` available, `uv run archex benchmark determinism-economics --sessions benchmarks/determinism_economics_r6_1/sessions.json --output benchmarks/evidence/s7-determinism-economics-r6.1.json --preregistration-commit <merged-SHA>` regenerates the artifact from the frozen fixture; `uv run archex benchmark validate --kind determinism-economics-r6-1 --input benchmarks/evidence/s7-determinism-economics-r6.1.json` exits 0; an independent fixture review recomputes every rendered-prefix SHA, routing assertion, receipt linkage, and prompt-cost isolation before the data-generating run; full gate green. |
| Design reevaluation | Before PR-1, confirm from official OpenRouter documentation and its live `anthropic/claude-opus-5` model endpoint that explicit cache control, cache-write/read usage fields, and a separable prompt-cost field are supported; record model, provider-routing policy, upstream provider, price schedule, response-output handling, and retrieval timestamp in the pre-registration. Before PR-2, confirm `OPENROUTER_API_KEY` supplies quota without printing a secret and that every prewarm/replay receipt identifies the Anthropic upstream with no fallback response. A price change invalidates only dollar interpretation; absent cache-use or separable prompt-cost fields, an inability to pin the upstream, a fallback, changed cache semantics, or missing credentials blocks the run. No local counter substitutes for provider-accepted cache-write evidence. |
| Risks & rollback | Risk: OpenRouter routing, credential, quota, pricing, cache TTL, response-output behavior, or exact-prefix semantics prevents a valid measurement. Mitigation: fail closed on the requested routing policy, resolved upstream metadata, cache-use provenance, and prompt-cost isolation; never reinterpret a mechanism-not-fired cell as a null. Risk: the seed-recorded ANN-style comparator is mistaken for a retrieval treatment. Mitigation: document and validate that it reorders exactly the same frozen chunks. Rollback: revert the benchmark-only stack; product ordering remains unchanged. |
| Est. PRs | 4, after the R6.1-DG-003 docs-only reconciliation PR merges |
