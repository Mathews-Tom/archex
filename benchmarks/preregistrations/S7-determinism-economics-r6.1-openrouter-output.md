# S7 — Determinism as Prefix-Cache Economics, R6.1 OpenRouter Output-Accounting Cell

## Study identity

- **Spike ID and title:** S7 — Determinism as Prefix-Cache Economics, R6.1 OpenRouter/Anthropic output-accounting cell
...
This is a new protocol. It supersedes the unrun, first-party direct-Anthropic protocol in PR #596 and the unrun zero-output OpenRouter protocol in PR #598. It reuses neither a fixture, receipt, artifact, decision, cache-use result, nor data from cancelled R6 or PR #593. The feasibility probes recorded before this document are not study data and are not reused. No study data exists when this document is committed.
...
All 12 sessions have the fixed label `resolved: true`, so cost per resolved task divides every arm by the same constant and cancels from this primary comparison. For each arm, input-side USD cost is the sum of the provider-observed `usage.cost_details.upstream_inference_prompt_cost` across measured turns. The fixture records `prompt_tokens`, `cache_write_tokens`, `cached_tokens`, completion tokens, total `cost`, prompt cost, and completion cost for audit; no local price calculation substitutes for the observed prompt-cost field. Completion tokens and their cost are excluded from the metric because OpenRouter returned them despite `max_tokens: 0`; response content never enters a later request. Larger positive values favor deterministic ordering. Cache-hit rate, returned `cost`, total input tokens, per-turn rendered-prefix identity, and absolute input-side prompt cost are exploratory diagnostics. Eligibility prewarm/replay calls never contribute to a measured arm's cost.
...
OpenRouter cache semantics and usage source: <https://openrouter.ai/docs/guides/best-practices/prompt-caching>, retrieved 2026-07-29T19:19:08Z. OpenRouter provider-selection source: <https://openrouter.ai/docs/guides/routing/provider-selection>, retrieved 2026-07-29T19:19:08Z. Canonical model endpoint: <https://openrouter.ai/api/v1/models/anthropic/claude-opus-5-20260723/endpoints>, retrieved 2026-07-29T19:19:08Z. Selected model: `anthropic/claude-opus-5`; requested routing is `provider.only: ["anthropic"]` and `allow_fallbacks: false`; required resolved receipt field is `provider: "Anthropic"`. The endpoint's base input price is `$0.000005` per token; cache write is `$0.00000625` (1.25×); cache read is `$0.0000005` (0.1×). The documented Anthropic cache TTL is five minutes. A changed price invalidates dollar interpretation only; it cannot change cache-hit interpretation.

The fixture builder must obtain an OpenRouter prewarm receipt showing a nonzero `usage.prompt_tokens_details.cache_write_tokens` for every exact rendered prefix before data generation. It must replay the same SHA-256 prefix before the five-minute TTL elapses and obtain nonzero `usage.prompt_tokens_details.cached_tokens`: one deterministic prefix plus three per-turn prefixes for each comparator in each of 12 sessions, or 84 prefixes total. Each committed receipt must carry its rendered-prefix SHA-256, model, requested routing object, returned `provider`, generation ID, returned `usage`, total cost, separable `cost_details.upstream_inference_prompt_cost`, completion cost and token fields, request and response timestamps, source revision, and fixture digest. Completion cost must be recorded but excluded from every input-side metric and response content must never enter a later request. It must record no fallback response. Missing, stale, mismatched, wrong-provider, fallback, or required-zero receipts fail fixture validation; no local token estimator is eligibility evidence.
