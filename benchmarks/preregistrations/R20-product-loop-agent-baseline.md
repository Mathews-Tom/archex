...
- **2026-09-10 — Harness hardened after the run, with no effect on any recorded cell.** A post-run audit found four gaps between this document and the implementation, all fixed without touching the 114 artifacts or the derived analysis, which still regenerates byte-for-byte: the runner now refuses a dirty archex tree and reports the source revision it ran from; the agent version is re-checked before every cell rather than once per sweep, so a mid-campaign auto-update aborts instead of splitting the population; the Graft MCP entry is recorded verbatim rather than reconstructed from two fields; the base allowlist is emitted in the order this document writes it; and a deadline-terminated cell whose transcript cannot be parsed now recovers its reported cost from the raw bytes, so its spend counts against the ceiling as this document requires instead of being charged zero. None of these applies retroactively to the published run, which had no deadline-terminated cell, spent `$7.74` of `$25`, and ran from a clean tree at `e0300bae` against a version-pinned binary with auto-update disabled.

- **2026-09-10 — Cardinality cap corrected to count resolved paths, after 16 cells existed.** The measured run was stopped at cell 16 because every Graft cell was scoring `answer_over_broad` while every Archex cell scored normally. The cause was an implementation defect, not a product difference: Graft's shipped `Stop` hook appends a token-savings footer (`🌱 graft saved ~151k tokens this turn`) *after* the `FILES:` block, and the extractor applied the cap to raw whitespace-separated tokens before normalization discarded the non-paths. A Graft answer naming exactly six real files was therefore counted as thirteen entries and zeroed. The frozen rule already says the block "may name at most `K = 6` **paths**", so the fix — normalize and resolve first, then apply the cap — makes the implementation match the pre-registered text rather than changing it, and it is arm-neutral because it discards any non-path token from either arm. A regression test pins the exact observed footer.

  **Disposition of the 16 cells:** discarded unread. No aggregate, mean, interval, or per-arm comparison was computed from them, and they are not resumed or reused; the measured run restarts from an empty results directory over all 114 planned cells. Their modelled cost was `$1.16`. They are recorded here because data briefly existed before the correction, and that fact belongs in the record even though no result informed the change.
