...
- **Intent token budgets cut to their measured saturation point.** `archex query` and the MCP context tools route an unspecified budget through per-intent presets, and two of those presets were asking for tokens that bought nothing. `architecture_broad` drops from 8192 to 3072 and `cli` from 3072 to 1024, moving the mean delivered bundle from 6058 to 2834 tokens and from 1536 to 985 respectively. Required-file recall, missed-required-task rate, and region recall are identical at both values in each case, and the follow-up read cost does not move at any rung — because required-file recall does not move — so the smaller bundle is a strict gain in token efficiency after completion rather than a trade. `general`, `debugging`, `definition_lookup`, and `usage_search` are deliberately left above their file-level saturation: their region or line recall keeps improving with budget (debugging goes 0.613 to 0.966 between 3072 and 8192), so those tokens buy real within-file coverage for the intents whose consumers need exact lines, and a blanket cut would have removed it. The caller ceiling stays at 8192; only the per-intent caps move, and an explicit `--token-budget` is unaffected. Measured over 66 tasks × 9 budgets on the product `archex_query` path with only the budget varying; dispositions and per-intent gate inputs are recorded in `docs/RETRIEVAL_DEFAULT_DECISIONS.md`.

### Added

- **Scope-aware monorepo ranking, benchmark-only.** `archex benchmark scope-aware-run` and `archex benchmark scope-aware-report` execute and render R26 Candidate A: deterministic scope discovery with repository-wide IDF, per-scope normalization, a participation gate, byte-identical retrieved-and-packed context on the single-scope fallback, and searched/included/rejected-scope receipts. The lane is never the product default and registers no MCP surface. Its frozen campaign has since returned a terminal **EVIDENCE NO-GO** over 4,128/4,128 declared cells — treatment-minus-control required-file recall `+0.029053` with a 95% interval of `[-0.002930, +0.059326]`, failing the minimum worthwhile gain and the binding `no_new_zero_recall` invariant on 110 new zero-recall tasks, while passing non-inferiority and the 3000 ms warm-p95 limit.
- **Agent token-headroom measurement.** `scripts/swebench_pro_token_headroom.py` decomposes where a coding agent's billed input tokens actually go, from published SWE-agent trajectories that need no credentials, no model, and no container. Over 165 trajectories across 11 repositories, repository-clustered: search accounts for 2.9% of one frontier model's billed input against 14.4% for another, and the share a retrieval tool could displace is 17.9% `[13.0, 23.1]` against 49.6% `[37.4, 63.1]`. The intervals do not overlap, so retrieval headroom is a property of the model's tool-use policy rather than of the benchmark. Because full-history scaffolds re-send every earlier observation, a front-loaded bundle is charged once per remaining turn and only pays below a break-even size, computed here as 6,501 `[4,541, 8,550]` and 18,941 `[14,935, 22,819]` tokens respectively.
- **Bundle budget ladder sweep.** `scripts/archex_bundle_budget_curve.py` runs the product path across nine budgets with only `token_budget` varying and reports saturation both in aggregate and per query intent, against each intent's shipped preset. This is the evidence the budget cut above rests on.
- **Filename-alignment boost ablation.** `scripts/path_boost_ablation.py` measures what `_path_alignment_boost` is worth by running the corpus twice with the multiplier live and neutralised. It is strongly net-positive — required-file recall `+0.0944` `[+0.0145, +0.1401]`, F1 `+0.1156`, line recall `+0.0214` `[+0.0066, +0.0444]`, while also shrinking bundles by 541 tokens and follow-up reads by 2,708 — so a narrow failure mode observed on caller-localization queries is not grounds for weakening it.
- **Query-intent corpus coverage.** The benchmark corpus gains two self-repo caller-localization tasks for the `usage_search` intent, which previously had none, and region labels on all five `definition_lookup` tasks, which previously had none. Both intents were held at their existing budgets as a result, on measured evidence rather than on its absence.
...
- **Verified same-repository worktree index seeding.** A fresh linked worktree now bootstraps from a same-repository index instead of paying a full cold build. Identity is proven twice — candidates come from `git worktree list --porcelain -z`, then each is re-interrogated from its own directory and must report the same `--git-common-dir` — and only `git rev-parse` is ever run inside a candidate, so no repository-configured hook, fsmonitor, or filter program participates in establishing identity. Submodules, bare repositories, and `--separate-git-dir` checkouts are refused structurally, because all three report their Git directory and common directory as equal. What transfers is exactly the seed's `index.db` content, copied through SQLite's backup API over a read-only connection, then delta-synchronized to the destination tree and published marker-last; no lock, WAL/SHM, status, edit-state, session, metrics, or settings path is read at all. Every failure is a named disposition rather than an error, so seeding never fails the command that asked for it, and `Config.worktree_seed` (machine-level only) turns it off. Seeding refuses a destination whose index config enables `vector` or `splade`, because transferring embedding state across a delta needs the embedder pipeline ordinary delta indexing owns; both are off by default. Measured on this repository: a seeded worktree indexes in 2.1 s clean and 2.6–4.1 s after a delta, against 8.9–15.6 s for a full build, and a second `archex index` there reports `cached` in 74–106 ms. See [Worktree Index Seeding](docs/WORKTREE_INDEX_SEEDING.md).
- **Seed strategy reporting.** `archex index` names the strategy it used (`seeded`, `cached`, `delta`, or `full`) with the seed source, sync mode, and file count, and `--timing`/JSON output carries five new `PipelineTiming.seed_*` fields, so a fast index can be attributed rather than guessed at.
...
- **Orientation profile benchmark.** `archex benchmark orientation` measures both onboarding profiles over one graph artifact and a frozen self-repository task population, reporting context tokens, exact-path hits, locatable and unlocated expected files, completeness, mean locator breadth, a declared exploration-call model, files named out of the whole indexed corpus, and each profile's self-reported omissions in the unit each section counts. It is offline, model-free and agent-free, so the report regenerates byte-for-byte and its JSON re-renders to its markdown exactly. Measured on this repository: the compact profile at 900 tokens costs 830 tokens against 2859 for the full profile (−71.0%), with completeness unchanged at 1.0000 and mean locator breadth halved (103 against 198 files per handle). Two results are reported rather than buried: the compact profile names 17 files exactly where the full profile names 127, because it is a summary that reaches the same completeness through prefixes; and the modeled exploration-call figure moves the wrong way (46 against 32), because that model charges one unit per distinct locator directory and so penalises specificity. No agent-observed call reduction is claimed anywhere — R20's telemetry is whole-session with no orientation-phase segmentation. What the comparison does establish about honesty is one-sided: the full profile declares no omissions while silently truncating 524 public interfaces, 680 hotspots and 375 test files; the compact profile declares every section it summarised. Evidence: `benchmarks/orientation/R24_ORIENTATION.md`, `benchmarks/evidence/r24-compact-orientation.json`.

### Security

- **A repo-local index is only reused when archex's own authenticated marker proves it.** `.archex/index.db` and `.archex/settings.toml` are ordinary files a published repository can commit, and reuse previously rested on metadata stored *inside that database*. Reproduced end to end with the shipped `0.29.0` CLI on a real clone before the fix: a repository that commits a poisoned index plus a `settings.toml` gets `archex query` on a victim's clone to answer out of attacker-authored chunk rows attributed to real file paths. The exploit needs no local write access — only that the victim clone the repository. Reuse now requires `.archex/index.meta` carrying `sha256("<resolved absolute path>@<commit>")` plus an HMAC keyed to machine-local state, and the requirement holds in **every** cache layout, because `cache_dir` is itself settable from a committed `settings.toml`. `inspect_project_status` no longer opens an unmarked index either: it reports `unprovenanced`, which closes `archex status`, `doctor`, and the tool-call hook that gates on freshness and would otherwise have injected attacker-chosen symbol rows into an autonomous agent's context. Accepted cost, stated rather than hidden: any repository whose cache predates authenticated markers re-indexes once, and a moved or renamed checkout pays one full re-index instead of a delta; both self-heal on the next run.
...
- **Non-blocking post-edit impact loop.** A shared post-edit service records edited paths in a bounded, versioned state document at `.archex/post-edit-state.json`, synchronizes through the existing delta/full fallback, and emits a file-level impact summary only when the resulting index generation is fresh. Hooks fail open at the client boundary — an edit is never blocked or failed — and fail closed on freshness, so a stale index produces no impact claim rather than a confident wrong one. Risk is labelled file-scoped; no symbol-call blast radius is claimed. Adapters ship for `claude-code`, `codex`, `omp`, `pi`, and `opencode`; `cursor` is explicitly unsupported because its only event carrying an edited path documents no output field.
- **Client-neutral freshness and status surface.** `archex status --cached` and a versioned bounded snapshot at `.archex/status-snapshot.json` expose index freshness, dirty/pending edits, watch state, last successful sync, and receipt completeness. The writer owns `fresh`, `dirty`, and `pending`; a reader adds `missing`, `corrupt`, `unsupported`, and `stale`. Renderers read only the cached document — a POSIX `sh` status line for Claude Code and a byte-identical extension module for omp and Pi — and construct no `IndexStore` and launch no child process per repaint. Status never reports approximate token savings. `codex`, `cursor`, and `opencode` are refused with their upstream reason and pointed at `archex status --cached`.
- **Interactive Explorer navigation.** `archex explore` gains a node-search view over the same exact-then-fuzzy resolution `archex graph neighbors` uses, per-edge orientation relative to the seed (`out` — the seed side depends on the far side, `in` — the far side depends on the seed side, `lateral` — equal distance, no direction relative to the seed), and typed-edge filtering with per-type facets computed over the unfiltered traversal so a filter can always be widened. Orientation is derived from hop distance over the edges the bounded traversal already returned; no adjacency the graph does not contain is constructed. The filter narrows the bounded neighborhood rather than the whole graph, and the page says so. Interactivity is plain links and one `<form method="get">`: pages remain script-free under a Content-Security-Policy that grants no `script-src`.
- **Self-contained Explorer export.** `archex explore ARTIFACT --export DIR` writes the same views as sibling HTML files that open from a `file://` URL with no server, no session token, and no network — inline CSS only, zero script, zero remote stylesheet/font/image, relative links only. Controls no server can answer are omitted and their absence explained; in their place the bundle ships a degree-ranked index of every exported node plus a pre-rendered depth-1 neighborhood page per high-degree node, whose orientation column still separates inbound from outbound dependencies. Both caps (200 node pages, 1000 index rows) are reported on the rendered page rather than applied silently.
- **Read-only review artifacts in CI.** `.github/workflows/report-diff.yml` now also exports the architecture graph and the offline Explorer bundle and uploads them alongside the canonical `report-diff.json`, with a job-summary pointer. The bundle is a projection of the uploaded canonical artifacts produced by the same view models and renderers the local server uses, not a second analysis. Delivery is build artifact plus step summary only: no pull-request comment, no write scope, and therefore identical behavior on a fork's read-only token.
- **`GraphQuery.node_summaries()`.** A read-only accessor returning every deduplicated node with the degree the engine computed for it — no traversal, no ranking, no query — so a caller enumerating nodes reports the same identity and degree as every other graph surface.

### Changed

- **Explorer inputs are bounded.** Every Explorer view was already row-bounded, but the canonical loaders validated a whole document before any view ran. `archex explore` now refuses a report artifact above 32 MiB or a graph above 128 MiB, 200,000 nodes, or 400,000 edges, naming the limit exceeded instead of rendering a partial view. The byte check runs before the parse. These limits live in the Explorer, so `archex report diff` and `archex graph export`/`inspect` semantics are unchanged.
- **Every workflow declares its token permissions explicitly.** `ci.yml` — which runs on every pull request — and `dogfood.yml` previously declared no `permissions:` block and silently inherited the repository's default token scope. Both now declare `contents: read`. The workflow audit in `tests/test_report_ci_workflow.py` covers every file in `.github/workflows/` (both `.yml` and `.yaml`), inspects job-level blocks as well as workflow-level ones, and rejects any write grant that is not explicitly allowlisted — the only one is `packages: write` on the image-publish workflow, which reaches a container registry and never the repository.
- **Benchmark-only additions (no product default change).** A pinned Graft comparison adapter, generalized graph-memory lanes, and an isolated product-loop runner ship with their preregistrations and published evidence. These are benchmark surfaces: no retrieval lane was promoted, no default changed, no language tier was added, and no MCP tool was added.

### Fixed

- **Inbound graph edges survive a delta reparse.** `IndexStore.delete_and_insert_for_files` deleted edges matching the reparsed files by `source` **or** `target`. Only modified files are reparsed, and a reparse regenerates only that file's *outgoing* edges, so every edge pointing *at* a modified file was deleted and never rebuilt — permanently, and silently. Reproducible without any archex API: index a repository where `consumer.py` imports `core.py`, touch `core.py`, index again, and both inbound edges are gone. Everything downstream of the dependency graph — impact, graph expansion, centrality — understated fan-in for any file edited since its last full index.
- **`archex status --cached` resolves from a subdirectory.** It did not walk up to the repository root, so from any subdirectory it reported `missing` while the client renderers reported the true state — on the surface documented for clients that have no status line.
- **Explorer edge rows are no longer double-counted.** `GraphQuery.neighbors` walks with `direction="both"` from every frontier node, so at depth greater than 1 an edge between two traversed nodes is legitimately selected once from each endpoint. The rendered table listed it twice and per-type facets counted it twice. Rows are now deduplicated for display only, keyed on every field a row shows; the traversal's own `truncated`/`omitted_edges` budget is unchanged.
- **Explorer node index and neighborhood pages agree on degree.** The static node index re-counted edge endpoints while `GraphQuery` skips an edge unless both endpoints exist as nodes. A dangling edge — ordinary output whenever a file imports something unindexed or external — inflated that node's degree, so the index's degree column and its highest-degree page ordering contradicted the same node's own neighborhood page.
- **Re-exporting the Explorer into a reused directory no longer leaves the previous bundle.** Per-node filenames key on the current artifact's node ids, so an earlier export's pages survived under different names: unlinked from the regenerated index but still openable from a `file://` URL, carrying a different change's identity and revision into a review bundle. The export now owns its destination.
...
- **R5: retrieval-gated MCP tool disclosure.** `archex mcp` now advertises only the two retrieval entry points (`context`, `query_repo`) until the client calls one, then advertises everything and sends `notifications/tools/list_changed`. For every turn before the client first retrieves, the fixed per-turn tool-schema cost drops from **3 859 tokens to 765, an 80.2% reduction** (`benchmarks/results/r5_mcp_retrieval_gated_disclosure/DECISION.md`). Safe as a default because the server declares the `listChanged` capability and retries a failed announcement, so a client is entitled to act on the notification; narrowing what is *advertised* also never narrows what is *callable*, which keeps hardcoded callers and agent files that name tools directly working through a closed gate. `archex mcp-schema-size` now reports tokens as well as characters, since every schema-size target this project sets is stated in tokens and the command measured only characters; and a bare invocation now measures the gated default rather than scope `all`, reporting the pre- and post-retrieval cost together, so the command that grades the milestone no longer reports the pre-R5 figure. `install-client`/`setup --no-disclosure` writes the opt-out for a client that needs every tool visible before it will call anything; the default stays implicit, so generated configs are byte-identical to pre-R5 ones. Note `--tools` does not disable the gate — it bounds what is advertised once the gate opens, so `--tools all` still starts minimal. No tool was added, removed, or changed in behavior. Corrected at the design gate: the plan's `~6 000`-token baseline was never this repo's; the measured full surface is 3 859 tokens.
- **R4: corpus validity audit.** Added `archex.benchmark.corpus_audit` and `archex benchmark validate --kind corpus-audit`: leakage scoring in three separately reported tiers, repository-cluster statistics with a size-weighted design effect, a held-out-set audit, and a cluster-bootstrap power simulation calibrated against R3's measured interval before it projects anything. The artifact schema recomputes that calibration rather than trusting its verdict field, and cross-checks the leak rates and cluster sizes against the task count it claims. Published in `benchmarks/evidence/s2-corpus-validity.json` with `benchmarks/corpus_audit/DECISION.md`. The finding: at 64 tasks over 16 repository clusters a literature-sized +4.88-point effect has power 0.108 against a 0.80 target, the detectable effect is +25 to +30 points, and the mean 95% interval is 34 points wide on a metric bounded at 100 — so every archex null measured on this corpus was uninterpretable by construction. Reaching 80% power for that effect needs roughly 2 048 tasks against the 64 available. Two further findings: the declared held-out set overlaps the task corpus completely and no code path enforces it, and symbol-tier query leakage is 29.7%, which confirms rather than corrects the 8-of-21 figure previously recorded for the `loc_*` family.
- **R3: external-replication evidence contract.** Added `archex.benchmark.replication`, a schema for recording reproductions of *published* results in their original authors' own reference setups, and `archex.benchmark.replication_analysis`, a paired cluster bootstrap over repository clusters. Surfaced as `archex benchmark validate --kind replication --input <artifact>`; `--kind evidence` is unchanged and still takes a directory. Every arm carries the harness commit, dataset revision, model revisions, library versions, and command, and is labelled `replication`, `adaptation`, or `original`. The validator re-derives each arm's verdict from its own delta, pre-registered band, and interval through the single `derive_verdict` rule the analysis also uses, and rejects an artifact whose recorded verdict does not follow — so a miss cannot be filed as a pass, an interval spanning zero cannot be rounded up to one, and a band that no longer contains the delta it was centred on is refused outright. An arm whose reference setup was never released is recorded `unrunnable`: neither a pass nor a fail, and never counted toward a pass. The public replication artifacts record Gate A as **FAIL**: the [RLCoder arm](benchmarks/evidence/s0-rlcoder-replication.json) reproduced +2.8125 EM points against a pre-registered band of [+2.88, +6.88], and the [cAST arm](benchmarks/evidence/s0-cast-replication.json) is `unrunnable`; R7 through R16 are cancelled.
...
- **M7 (disabled by default — see `benchmarks/results/m7_runtime_evidence/DECISION.md`): conditional revision-bound runtime/coverage evidence infrastructure.** Added `archex.integrations.runtime`: a `CoverageEvidenceProvider`/`RuntimeProfileEvidenceProvider` contract (`probe()`/`collect()`) with `CoverageXmlProvider` (reads a previously generated Cobertura `coverage.xml` report) and `RuntimeProfileProvider` (reads a previously collected folded-stack profile) implementations. Every evidence record is revision-bound; a provider whose evidence was collected against a different revision than the one currently being analyzed reports an explicit `STALE` receipt rather than applying mismatched evidence. `IndexStore` persists provider receipts, coverage evidence, and profile evidence; `ContextReceipt.runtime_providers` surfaces them on the query cold path. `AnalysisArtifactV1`'s `SymbolCandidate`/`TestCandidate` display runtime-sample-count/coverage-line-rate provenance and staleness, rendered in both the HTML and Markdown diff reports. `IndexConfig.runtime_evidence_providers` gates the channel; empty by default, so no provider runs and zero bytes of retrieval behavior change. Evidence is deliberately read-only in this milestone — never consulted by search, ranking, or candidate expansion — so the `archex_query_runtime_evidence` benchmark lane measured byte-identical results to `archex_query` across the 24-task self-repo family even with real, revision-verified evidence present; the channel stays disabled pending a future milestone that consumes this evidence in a performance-localization or test-risk-aware retrieval strategy.
- **M8 (disabled by default — see `benchmarks/results/m8_repository_memory/DECISION.md`): conditional repository-memory (local-history) evidence infrastructure.** Added `archex.integrations.history`: a `GitLogEvidenceProvider`/`OperatorRationaleEvidenceProvider` contract (`probe()`/`collect()`) with `GitLogHistoryProvider` (collects local commit history live from `.git`, bound to a 200-commit window, never a remote call; extracts issue/PR references from commit subjects via local regex) and `OperatorRationaleProvider` (reads previously supplied, revision-bound operator rationale) implementations. `archex.integrations.history.eligibility.evaluate_history_eligibility()` gates whether collected history is ever surfaced: density/linkage/relevance must each clear their predeclared thresholds before a report presents it, and reports never call temporal coupling a dependency.
- **M9 (disabled by default — see `benchmarks/results/m9_documentation_status/DECISION.md`): conditional documentation-graph evidence infrastructure plus a dimensioned, evidence-linked status card.** Added `archex.integrations.docs`: a `DocLinkEvidenceProvider`/`AdrEvidenceProvider`/`OwnershipEvidenceProvider` contract (`probe()`/`collect()`) with `DocLinkProvider` (scans README/`docs`/`.docs` markdown already on disk for links resolving to a real file under the repository root), `AdrProvider` (reads a conventional ADR directory), and `OwnershipProvider` (reads a conventional CODEOWNERS-style manifest) implementations — none ever folded into `DependencyGraph`; documentation evidence is association, never a code dependency. `archex report status-card SOURCE --format json|markdown` renders a dimensioned status card (Documentation linkage, ADR provenance, Ownership coverage, Release & CI evidence) that is structurally never scored — no field anywhere aggregates dimensions into a composite grade — and `archex report release-artifact SOURCE` bundles archex's own version/schema-compatibility facts with that status card into one per-release evidence document. `.github/workflows/status-card.yml` is a new immutable-pinned, read-only CI example publishing both.

### Changed

- **R2: truth-in-claims correction — published savings numbers re-baselined, the self-repo row withdrawn, and the scorecard's completion metric renamed.** No number changed how it is computed and no artifact was deleted; what changed is how numbers are labelled and published. (1) The savings headline in `README.md`, `docs/LOCAL_METRICS.md`, and `archex metrics summary` now points at `savings_pct_vs_targeted_read` (`archex.metrics.math`) — savings against a realistic targeted read of the matched line ranges plus a small context window — instead of the full-file-paste figure, which models a strategy no agent uses and reads roughly six times higher (14.9% against 90.1% on a representative single-`query` ledger row). `archex metrics summary` prints the targeted-read line first, labeled `headline`, and the full-file line second, labeled `compression`; both values and both baselines are unchanged. All three surfaces also now state that the targeted-read baseline is absent, not zero, for events without line spans: `record_query_usage` supplies it, `record_scout_usage` does not, and the summary sums a null as `0`, so a scout-only ledger renders `Savings vs targeted: 0.0%`. `docs/WHY_ARCHEX.md`'s four worked figures now name the whole-file-read baseline on each line and are labeled illustrations rather than measurements. (2) The `self` corpus is withdrawn from every currently published cross-tool figure. archex's own generic keywords (`index`, `query`, `config`) match across its entire source, so the naive grep/read baseline reads a median of 32.5 units per self-repo task against 3.0 on the external corpora; the published README range is now 95.4%–97.2% over `external-comprehension` and `external-localization` only, and every remaining savings figure names its baseline in the same sentence. All 16 self-repo comparisons remain in `benchmarks/cross-tool-efficiency/cross-tool-comparison.json`. Shipped changelog entries are historical records and are not rewritten, so the `0.15.0` entry still quotes the older self-inclusive range and is superseded by these figures. (3) `benchmarks/cross-tool-efficiency/README.md` (new) and `docs/LOCAL_BENCHMARK_EVIDENCE.md` document the blind-read semantics of `tokens_at_recall` (`archex.benchmark.cross_tool`) — it charges every unit in a fixed lexical ranking with no triage step, though it does stop at the unit completing the required set, an oracle stop that makes the modelled naive cost a floor — together with the measured units-read distribution over the 52 comparable tasks (median 5.5, mean 18.0, max 164; self 32.5/48.4/164, external corpora combined 3.0/4.4/18, archex 3.0/3.1/8). (4) `ScorecardRow.downstream_success_rate` is renamed `required_file_completeness_rate` (`archex.benchmark.scorecard`), the scorecard markdown column header reads `Required-File Completeness`, and every rendered scorecard now carries the metric's definition beneath its tables. It never measured downstream task success: on every default path each task contributes `1.0` when no required file was missing and `0.0` otherwise, a function of required-file recall with no model in the loop. In the opt-in `benchmark bundle-eval` lane a non-null `bundle_only_success` takes precedence and the value is answer correctness instead; that value comes either from the operator's evaluator command or, when it omits the field, from archex comparing the evaluator's `answer` to the task's `expected_answer`. The per-result scoring shared by the scorecard and the promotion gate is now one function (`archex.benchmark.models.completion_outcome_score`), and `check_fixed_agent_non_regression`'s documentation is relabelled to match while keeping its `fixed_agent_success_regression` metric string as a stable contract. `docs/external-quality-frontier.md` and `docs/RETRIEVAL_DEFAULT_DECISIONS.md` are relabelled; their values are unchanged. **Breaking (benchmark-only):** `M3ScorecardArtifact.artifact_version` moves from `1` to `2` — a previously saved scorecard JSON no longer loads (pydantic reports the renamed field as missing) and must be regenerated. No scorecard artifact is checked in.

## [0.24.0] - 2026-07-24

### Added

- **M11: reduced MCP tool-schema context overhead via tool-scoping and a consolidated `graph_query` tool.** Added a tool-scoping mechanism for the MCP server: `archex mcp --tools <scope>` and `install-client`/`setup --tool-scope <scope>` accept `all` (default, unchanged), a named profile (`core` excludes the five `graph_*` tools, `graph` is exactly those five), or a comma-separated explicit tool-name allowlist — `list_tools()` advertises only the scoped subset while `call_tool` dispatch stays unaffected, so no currently-registered client config or tool name breaks. Added `archex mcp-schema-size [--tools SCOPE] [--format text|json]`, reporting total/per-tool serialized schema size for a scope. Trimmed the heaviest tool descriptions (`get_impact`, `explain_target`, the `graph_*` cluster), cutting the unscoped baseline from 14270 to 13907 characters. Added a consolidated `graph_query` tool (`operation`: lookup/neighbors/path/stats/hubs) dispatching through the exact same handlers as its five `graph_*` predecessors — which remain registered, functional, and unchanged for this milestone's deprecation window — with byte-identical results for equivalent inputs. `--tool-scope core` now registers 14 tools at 12130 characters, 15.0% below the original pre-M11 unscoped baseline of 14270, while still covering every capability including graph inspection via the single `graph_query` tool. Full before/after numbers: `benchmarks/results/m11_mcp_schema_overhead/BASELINE.json` and `DECISION.md`. No retrieval ranking, receipt, CLI, or Python API behavior changed.
...
This release combines three deferred release trains that were never individually tagged or published — `v0.20.0` (M2), `v0.21.0` (M3, M10), and `v0.22.0` (M4) — into a single cut under the release-preparation policy in effect at the time. Each entry below is labeled with the version it was originally planned for.
