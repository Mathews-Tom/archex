...
- **R2: truth-in-claims correction — published savings numbers re-baselined, the self-repo row withdrawn, and the scorecard's completion metric renamed.** No number changed how it is computed and no artifact was deleted; what changed is how numbers are labelled and published. (1) The savings headline in `README.md` and `docs/LOCAL_METRICS.md` now points at `savings_pct_vs_targeted_read` (`archex.metrics.math`) — savings against a realistic targeted read of the matched line ranges plus a small context window — instead of the full-file-paste figure, which models a strategy no agent uses and runs an order of magnitude high (14.9% against 90.1% on a representative single-`query` ledger row). Both docs also now state that the targeted-read baseline is absent, not zero, for events without line spans: `record_query_usage` supplies it, `record_scout_usage` does not, and `archex metrics summary` sums a null as `0`, so a scout-only ledger renders `Savings vs targeted: 0.0%`. (2) The `self` corpus is withdrawn from every quoted cross-tool figure. archex's own generic keywords (`index`, `query`, `config`) match across its entire source, so the naive grep/read baseline reads a median of 32.5 units per self-repo task against 3.0 on the external corpora; the published README range is now 95.4%–97.2% over `external-comprehension` and `external-localization` only, and every remaining savings figure names its baseline in the same sentence. All 16 self-repo comparisons remain in `benchmarks/cross-tool-efficiency/cross-tool-comparison.json`. (3) `benchmarks/cross-tool-efficiency/README.md` (new) and `docs/LOCAL_BENCHMARK_EVIDENCE.md` document the blind-read semantics of `tokens_at_recall` (`archex.benchmark.cross_tool`) — it charges every unit in a fixed lexical ranking with no triage step and no early stop, making the reduction an upper bound on the advantage over a grep/read workflow — together with the measured units-read distribution over the 52 comparable tasks (median 5.5, mean 18.0, max 164; self 32.5/48.4/164, external corpora combined 3.0/4.4/18, archex 3.0/3.1/8). (4) `ScorecardRow.downstream_success_rate` is renamed `required_file_completeness_rate` (`archex.benchmark.scorecard`) and the scorecard markdown column header reads `Required-File Completeness`. It never measured downstream task success: each task contributes `1.0` when no required file was missing and `0.0` otherwise, so it is a function of required-file recall with no model in the loop, the sole exception being the opt-in `benchmark bundle-eval` lane where an operator-supplied evaluator may report `bundle_only_success`. `docs/external-quality-frontier.md` is relabelled to match; its values are unchanged.
