...
- **M8 (disabled by default — see `benchmarks/results/m8_repository_memory/DECISION.md`): conditional repository-memory (local-history) evidence infrastructure.** Added `archex.integrations.history`: a `GitLogEvidenceProvider`/`OperatorRationaleEvidenceProvider` contract (`probe()`/`collect()`) with `GitLogHistoryProvider` (collects local commit history live from `.git`, bound to a 200-commit window, never a remote call; extracts issue/PR references from commit subjects via local regex) and `OperatorRationaleProvider` (reads previously supplied, revision-bound operator rationale) implementations. `archex.integrations.history.eligibility.evaluate_history_eligibility()` gates whether collected history is ever surfaced: density/linkage/relevance must each clear a predeclared threshold against the operation's own candidate files, or the channel stays disabled with an explicit reason — collected `TemporalCouplingObservation` co-change data is never treated as a dependency claim and never folded into the graph. `IndexConfig.history_evidence_providers` gates the channel; empty by default, so no provider runs and zero bytes of retrieval behavior change. `ContextReceipt.history_providers`/`.history_eligibility` and `AnalysisArtifactV1`'s `DiffFileChange`/`DiffAnalysis` surface the same decision. Evidence is deliberately read-only in this milestone — never consulted by search, ranking, or candidate expansion — so the `archex_query_history_evidence` benchmark lane measured bit-for-bit identical results to `archex_query` across the 24-task self-repo family; the channel stays disabled pending a future milestone that consumes this evidence in a history-aware retrieval strategy.
