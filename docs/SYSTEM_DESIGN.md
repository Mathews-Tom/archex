...
- **Retrieval eval frontier:** benchmark tasks may optionally declare `expected_regions`, each a path plus line range, or a path plus symbol/block handle, with a per-region `granularity` of `file`, `symbol`, `block`, or `line_range`. The real task corpus now has a maintained labeled subset (self-repo plus selected external framework tasks) with a checked-in `archex_query` baseline at `.archex/r0-labeled-baseline`; unlabeled tasks still report `null` for region fields. When labels exist, results report region recall/precision/F1, line recall/precision, ranked-region MRR/nDCG (over returned context order), and context-efficiency signals (`context_noise_ratio`, `useful_tokens`, `wasted_tokens`, `relevance_per_1k_tokens`) alongside the file-level metrics. The three measurement levels are complementary: **file-level** asks whether the right file was returned at all; **region-level** (symbol/block/line-range) asks whether the right enclosing code was returned; **line-level** asks how much of the reviewed code was actually relevant.
