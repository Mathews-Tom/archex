...
- **Retrieval eval frontier:** benchmark tasks may optionally declare `expected_regions`, each a path plus line range, or a path plus symbol/block handle, with a per-region `granularity` of `file`, `symbol`, `block`, or `line_range`. The real task corpus now has a maintained labeled subset (self-repo plus selected external framework tasks) with a locally-generated `archex_query` baseline (the run directory is gitignored); unlabeled tasks still report `null` for region fields. When labels exist, results report region recall/precision/F1, line recall/precision, ranked-region MRR/nDCG (over returned context order), and context-efficiency signals (`context_noise_ratio`, `useful_tokens`, `wasted_tokens`, `relevance_per_1k_tokens`) alongside the file-level metrics. The three measurement levels are complementary: **file-level** asks whether the right file was returned at all; **region-level** (symbol/block/line-range) asks whether the right enclosing code was returned; **line-level** asks how much of the reviewed code was actually relevant.
- **Issue-to-edit localization family:** benchmark tasks carry a `family` axis (`comprehension` or `localization`) that is orthogonal to `category` (repo type). The localization family (`benchmarks/tasks/loc_*.yaml`) is a small, maintained set of hand-curated SWE-bench-style issue-to-edit tasks — each an issue-style question plus `expected_regions` at file and symbol granularity pinned to a public release tag — that measures the real downstream fault-localization job (given an issue, find the files/functions to edit) rather than "how does X work" comprehension. It reuses the existing region-metric computation and changes no retrieval code path. `archex benchmark report` renders `reporter.format_localization_summary`, which groups tasks by family and reports file-level localization (required-file recall, MRR, and nDCG over returned file order) and region-level localization (region recall and ranked-region MRR/nDCG over returned context order) per strategy. Grouping is per task: every strategy result for a localization task — including the raw baselines — is reported under localization, the two families are tabulated in separate sections, and the report never publishes a single cross-family aggregate winner. The section renders only when a localization task is present, so pure-comprehension runs are unaffected.
- **Personalized structural-centrality lane:** `archex_query_ppr` is a benchmark-only strategy that keeps the BM25 retrieval and packing path but swaps the `structural` scoring term for query-personalized, confidence-weighted, importer→imported PageRank. The default `archex_query` lane still uses cached global `graph.structural_centrality()` unchanged. PPR seeds reuse the normalized BM25 seed scores and `SEED_EXPANSION_MIN`; PageRank runs only on a bounded seed neighborhood with hard node/frontier/hop caps, then records `centrality_variant`, personalization/fallback status, subgraph node/edge counts, and centrality latency for report provenance. It is in `AVAILABLE_STRATEGIES` only, never `DEFAULT_STRATEGIES`, and it remains promotion-ineligible unless the labeled warm-run gate in `docs/RETRIEVAL_DEFAULT_DECISIONS.md` clears.
- **Recency/churn prior lane:** `archex_query_churn` is a benchmark-only strategy that keeps the `archex_query` BM25 retrieval, graph expansion, packing, and default scoring weights, and adds a small per-file recency/churn prior as a bounded multiplier (`[1.0, 1.0 + max_boost]`, `max_boost = 0.05`) on the final score. The prior is intent-gated to `DEBUGGING` and `USAGE_SEARCH` only, so it can break ties and nudge ordering but never dominate relevance, and never touches the product default (`query` only applies it when a candidate supplies one). It is optional and artifact-backed: input comes from a checked-in `archex.churn.v1` per-file churn fixture or a full local clone's git history, and falls back to a neutral prior (multiplier `1.0`) whenever history is unavailable (shallow clone, single-commit repo, git failure) so the lane stays bit-identical to `archex_query` and deterministic. Strategy provenance records `churn_source` (`history`/`fixture`/`neutral_fallback`), the intent-gate decision, and the per-file priors applied. It is in `AVAILABLE_STRATEGIES` only, never `DEFAULT_STRATEGIES`, and the lane evaluates only on the gated-intent task subset.
- **Task-aware candidate lane:** `archex_query_task_aware` is a benchmark-only strategy that routes sparse-vs-dense retrieval by query modality (`pl_to_pl`/`nl_to_pl`/`mixed`) and budget tier (`tight`/`standard`/`large`). It runs BM25 first with a single confidence-gated dense expansion for code-heavy/mixed queries and a bounded hybrid pass for natural-language queries; confidence-aware fusion runs only on the hybrid/dense pass, the cross-encoder reranker is never run, and work is bounded to at most two retrieval passes. It is absent from `DEFAULT_STRATEGIES` and does not change the product query path; strategy provenance records modality, budget tier, routing decision, candidate caps, skipped steps, and whether fusion ran. `docs/RETRIEVAL_DEFAULT_DECISIONS.md` owns its default-promotion rule.
- **Post-retrieval compression lane:** `archex_query_compressed` is a benchmark-only strategy that runs the exact `archex_query` retrieval and packing path, then applies deterministic, low-risk compression after bundle assembly. Retrieval metrics stay attributable to the uncompressed retrieval set; compression metrics (`bundle_tokens_uncompressed`/`bundle_tokens_compressed`, `bundle_compression_ratio`, `required_context_passthrough_tokens`, `compression_hidden_required_region_count`, `token_efficiency_with_compression_and_completion`) describe the compressed output. Modes are deterministic text transforms only — `passthrough_required`, `structural_code_elision`, `comment_and_whitespace_slimming`, `large_literal_summarization`, `json_log_smart_crushing` — with no hosted calls, local model calls, or semantic summarization. Required/direct/high-confidence code passes through uncompressed; every compressed region keeps its original path/range/hash and an exact fetch-original handle. It is absent from `DEFAULT_STRATEGIES` and does not change the product query path. Compression never upgrades `context_complete`: **compression cannot make incomplete context complete**, and required regions hidden by compression are reported separately from retrieval misses and must stay `0`. `docs/RETRIEVAL_DEFAULT_DECISIONS.md` owns the lane verdict; `docs/CONTEXT_RECEIPTS.md` documents the compression metadata block.
- **Retrieval-aware packing lane:** `archex_query_efficiency_packed` is a benchmark-only strategy that runs the exact `archex_query` retrieval path, then re-packs the assembled bundle with a deterministic relevance-per-token packer. Each region is scored from intrinsic signals only (retrieval score, direct path/symbol match, graph distance and edge confidence, token count, compression eligibility/loss risk, scout/fetch handle priority, remaining budget) — never benchmark ground truth. Direct/high-confidence targets are preserved before optional context, smaller enclosing evidence is preferred over whole-file context except at large budgets or files with ≥2 high-confidence regions, low-risk regions may be compressed, and large low-score graph-distant context is anchored or skipped. Result fields (`packed_relevance_per_1k_tokens`, `packing_included`/`compressed`/`elided`/`skipped_regions`) plus the reused compression token fields record the outcome; provenance explains every include/compress/elide/skip decision, and compressed/elided rows keep the original content hash and fetch-original handle so the source stays retrievable. It is absent from `DEFAULT_STRATEGIES` and leaves the product query path unchanged; `docs/RETRIEVAL_DEFAULT_DECISIONS.md` owns the promotion verdict.
- **Advanced quality lanes (optional, benchmark-only):** Workstream 5 adds four experimental strategies — `archex_query_dual_transform` (deterministic structural+behavioural dual-query transformation, reciprocal-rank fused), `archex_query_bounded_rerank` (symbolic evidence rerank over a compact candidate set with hard candidate/latency caps, optional reuse of an already-loaded local cross-encoder only), `archex_query_summary_sidecar` (summary-first selection from an explicit offline-built sidecar with stale detection, then original-code retrieval), and `archex_query_graph_multihop` (bounded multi-hop dependency-graph expansion with edge-confidence/frontier/hop/token caps). Each is in `AVAILABLE_STRATEGIES` only, never `DEFAULT_STRATEGIES`. **Risk profile:** these lanes add cost the deterministic default does not pay — extra retrieval/expansion passes (added latency for dual-transform, multihop, and a cross-encoder rerank), an offline storage/index artifact plus staleness handling (summary sidecar), and graph-traversal complexity (multihop). They are deliberately held behind the default-switch gate until the benchmark frontier can attribute their wins and regressions, and none introduces hosted calls, telemetry, or mandatory network behaviour. Reports compare their added latency, token impact, storage/index cost where applicable, and quality metrics in the **Advanced Quality Lanes** table; `docs/RETRIEVAL_DEFAULT_DECISIONS.md` owns the promotion rule.
- **Competitive comparison (benchmark-only):** `src/archex/benchmark/competitive.py` and `archex benchmark headtohead competitive` render the public comparison across retrieval lanes (`archex_query` default plus optional benchmark-only archex candidates such as `archex_query_compressed` and `archex_query_efficiency_packed`), the external retrieval engine `ccc`, the raw-ripgrep/read baseline, Graphify graph/memory-layer lanes (`graphify_build_plus_query`, `graphify_query_warm`), and Headroom-style compression lanes (`headroom_only_on_raw_context`, `archex_plus_headroom`). The manifest tags each lane's `layer_type` (`retrieval`/`graph-memory`/`compression`/`baseline`) so compression/context-management and graph/memory layers are never presented as direct retrieval-equivalent winners; Graphify build cost and warm-query cost stay separate, and compression lanes contribute only a compression ratio while retrieval-quality columns stay `n/a` for them. Metrics are grouped by repo/task family and aggregate and are published only from checked-in artifacts.
