...
- **Retrieval eval frontier:** benchmark tasks may optionally declare `expected_regions`, each a path plus line range, or a path plus symbol/block handle, with a per-region `granularity` of `file`, `symbol`, `block`, or `line_range`. When labels exist, results report region recall/precision/F1, line recall/precision, ranked-region MRR/nDCG (over returned context order), and context-efficiency signals (`context_noise_ratio`, `useful_tokens`, `wasted_tokens`, `relevance_per_1k_tokens`) alongside the file-level metrics. The three measurement levels are complementary: **file-level** asks whether the right file was returned at all; **region-level** (symbol/block/line-range) asks whether the right enclosing code was returned; **line-level** asks how much of the returned context overlaps the expected lines. A strategy that returns the right file but the wrong lines is visibly worse than one that returns the expected region under the same budget. Region/line metrics are optional and computed only when labels exist; file-only tasks report `unknown`/`null` for them, and existing tasks, reports, and gates are unaffected. The metrics are deterministic and local — no LLM, hosted, or network calls are involved.
...
- **Competitive comparison (benchmark-only):** `src/archex/benchmark/competitive.py` and `archex benchmark headtohead competitive` render the public comparison across retrieval lanes (`archex_query` default plus optional benchmark-only archex candidates such as `archex_query_compressed` and `archex_query_efficiency_packed`), the external retrieval engine `ccc`, the raw-ripgrep/read baseline, and Headroom-style compression lanes (`headroom_only_on_raw_context`, `archex_plus_headroom`). The manifest tags each lane's `layer_type` (`retrieval`/`compression`/`baseline`) so a compression/context-management layer is never presented as a retrieval engine; compression lanes contribute only a compression ratio while retrieval-quality columns stay `n/a` for them. Metrics are grouped by repo/task family and aggregate with no aggregate-only winner claim: recall, required-file recall, region/line recall where labeled, precision/context-noise, token efficiency after completion, compression ratio, receipt accuracy, freshness, cold-start, and warm p50/p95 latency. Headroom is applied locally where reproducible or imported as version-pinned, SHA-256-stamped operator artifacts (`src/archex/benchmark/headroom.py`); `docs/ARCHEX_VS_COCOINDEX.md` owns the published numbers and every claimed value comes from checked-in artifacts under `benchmarks/headtohead/results/`.
