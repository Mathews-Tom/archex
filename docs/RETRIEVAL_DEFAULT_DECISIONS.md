...
Family expansion (Workstream L1 of `.docs/2026-06-21-localization-rerank-enhancement-plan.md`): the localization family was widened from 5 to 21 hand-curated, region-labeled tasks (target range 15–25) spanning Python (django, fastapi, flask, requests, httpx, pydantic, sqlalchemy, celery, click, pytest) and the multi-language repositories already in the corpus (express and react in JavaScript, gin in Go, tokio and mini-redis in Rust), each pinned to a public release tag (mini-redis to a public commit). This is the corpus that grades the conditional reranker's localization behaviour: an `n = 5` family was too thin to gate any promotion or opt-in decision, so the family is widened first and the reranker is graded against it. A labeled `archex_query` baseline over the full family was generated locally (the run directory is gitignored), with region recall, region precision, line recall, ranked-region MRR/nDCG, context-noise ratio, and relevance-per-1k-tokens populated for every task. No reranker result is recorded here; the reranker disposition is Workstream L2.
...
Disposition (local evaluation, 2026-06-21): the lane was run warm against the full benchmark corpus (48 tasks) with the cross-encoder pinned to `cross-encoder/ms-marco-MiniLM-L-6-v2`, comparing `archex_query` and `archex_query_conditional_rerank`; the run was performed locally (the run directory is gitignored). The ambiguity gate fired and the model applied on every task (`cross_encoder_status=applied`). Reranking changes only the order of the returned set — required-file recall, F1, `region_recall`, and `region_precision` are identical to `archex_query` on every task — so the effect appears only in the ranking metrics, and it is opposite-signed by corpus. On the external/real-world repos (24 tasks) ranked-region MRR rose `+0.14` (file MRR `+0.15`, nDCG `+0.10`, MAP `+0.13`) at p95 `1011 ms`; on the self-repo (24 tasks, identifier-dense comprehension queries where BM25 already ranks the right file first, base file MRR `~0.98`) ranked-region MRR fell `-0.10` and p95 was `3226 ms`, above the `3000 ms` budget. No cheap query-side gate signal (BM25 score CV, top-1/top-2 score margin, AvgIDF, or query intent) separates the corpus where the model helps from where it hurts, so the lane is not promotable via a per-query gate and its value is confined to the external/NL-localization distribution. It remains benchmark-only and does not clear the F1-based default-switch gate, since the returned set — and therefore F1 — is unchanged.

## Symbolic-blend reranker lane (opt-in localization mode)

`archex_query_symbolic_rerank` is a benchmark-only, **opt-in** retrieval lane (Workstream L2 of `.docs/2026-06-21-localization-rerank-enhancement-plan.md`). It is the output-side fix for the conditional reranker's self-repo regression: the cross-encoder demotes an exact-symbol match that BM25 already ranked first, and no cheap query-side gate (BM25 score CV, top-1/top-2 margin, AvgIDF, or query intent) separates the corpus where the model helps from where it hurts. Rather than predict when to run the model, this lane constrains its output. It keeps the `archex_query` BM25 retrieval, the conditional ambiguity gate (fires only on flat BM25 CV), and the 1500 ms latency cap, reuses the same in-process cross-encoder (it never downloads a model), and combines the model with the deterministic symbolic evidence signal already in `src/archex/benchmark/bounded_rerank.py` (`query_signals`/`symbolic_scores`/`evidence_score`, reused unchanged). A never-demote guard floors the result: a candidate with strong symbolic evidence (a direct path or symbol-name hit) can never fall below its pre-rerank rank. Two combination forms are gated behind a parameter (`SymbolicRerankMode`): `blend` (`alpha · norm(cross_encoder) + (1 − alpha) · norm(symbolic)`, the lane default at `alpha = 0.5`) and `guard` (cross-encoder order alone); the never-demote floor applies in both. The lane equals `archex_query` when no model is loaded, BM25 is confident, or the model pass aborts on latency, and records provenance (`symbolic_rerank_mode`, `blend_alpha`, `guard_fired`, the gate decision, and per-candidate model-vs-symbolic contributions). The product `archex_query` path and the pure-cross-encoder `archex_query_conditional_rerank` A/B baseline are unchanged.

Disposition (local evaluation, 2026-06-22): the lane was validated warm over the corpus including the expanded localization family (64 tasks: self=24, external-comprehension=19, external-localization=21) with the cross-encoder pinned to `cross-encoder/ms-marco-MiniLM-L-6-v2` via the BM25-only clean driver (no vector fusion lanes); the aggregated per-corpus evidence is checked in at `docs/symbolic-rerank-validation.md` (the raw per-task run directory is local-only and gitignored). The gate fired and the model applied on every task, and the lane only reorders, so recall, required-file recall, and F1 are identical to `archex_query` in every corpus. **The acceptance bar clears for the blend form.** On external-localization, ranked-region MRR is `0.7391`, at or above the pure-cross-encoder lane (`0.6782`) and far above the `archex_query` baseline (`0.4838`) — the localization win is preserved and exceeded. On the self-repo, ranked-region MRR is `0.5229`, at or above the `archex_query` baseline (`0.4745`), where the pure cross-encoder regresses to `0.3740` (`−0.1005`) — the self-repo regression is neutralized. p95 holds at or below `3000 ms` warm in every corpus where the lane fires (self `2685 ms`, external-comprehension `786 ms`, external-localization `1373 ms`).

Opt-in (do not flip the default): `archex_query_symbolic_rerank` (blend mode, `alpha = 0.5`) is the recommended **opt-in mode for localization workloads** — issue-to-edit "given an issue, find the file/region to edit" retrieval. Caveats: it carries the cross-encoder model cost (an opt-in local model the operator supplies and warms in-process; the lane is inert when no model is loaded), and its warm p95 is higher than the model-free `archex_query` (self-corpus p95 `2685 ms`, near but under the `3000 ms` budget), so it is a localization-scoped opt-in, **not** a product-default change. `archex_query` remains the product default; this lane does not change the default path, adds no hosted/telemetry/mandatory-network behavior, and the full switch gate (mean F1 ≥ `+0.05`, token efficiency, p95 ≤ `3000 ms`, no required-file/region regression) still stands — the lane does not move F1 because it never changes the returned set.
